...
/** OpenAPI 3.1 discovery for Hansala Agent API v1 — all agent routes. */
  ...
  const spec = {
    ...
    paths: {
      "/company": {
        ...
        patch: op(
          "Update profile (name, slug, category, tagline, …)",
          "patchCompany",
        ),
      },
      "/company/cover": {
      ...
      "/logo": {
        put: op("Upload logo (PUT)", "putLogo"),
      },
      "/logo/refresh": {
      ...
      "/case-studies/{id}": {
        ...
        patch: op(
          "Update case study (+ optional cover_image_url fetch)",
          "patchCaseStudy",
        ),
      ...
      "/case-studies/{id}/partners": {
        post: op(
          "Tag partner on case study (confirmed=false until human)",
          "tagCasePartner",
        ),
      ...
      "/client-confirmations": {
        post: op(
          "Email invite for client to confirm a case study",
          "clientConfirmation",
        ),
      },
      "/references": {
        ...
        post: op(
          "Create reference (client_name, service, started_year)",
          "createReference",
        ),
      },
      "/references/{id}": {
      ...
      "/verification/instructions": {
        get: op(
          "Snippet for one method (?method=meta_tag|dns|…)",
      ...
      "/team": {
        get: op(
          "List members + pending invites (requires team:manage)",
          "listTeam",
        ),
      },
      "/team/invitations": {
      ...
      "/group/invites": {
        post: op(
          "Invite existing company to group (they confirm)",
          "inviteGroupMember",
        ),
      },
      "/group/subsidiaries": {
        post: op(
          "Create auto-confirmed subsidiary (owner structure statement)",
          "createSubsidiary",
        ),
      },
      ...
      "/inquiries": {
        get: op("List profile inquiries", "listInquiries"),
      },
      "/inquiries/{id}": {
        patch: op("Triage inquiry status", "patchInquiry"),
      },
      "/analytics": {
        get: op("Profile analytics for keyed company", "getAnalytics"),
      },
      "/audit-log": {
        get: op("Own API audit trail", "getAuditLog"),
      },
      "/widgets": {
        get: op(
          "Embed variant catalog/snippets (requires settings:write)",
          "listWidgets",
        ),
      },
      "/widget-settings": {
