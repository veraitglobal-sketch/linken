...
export function IncomingPartnerRequests({ incomingPending }: Props) {
      ...
      <WorkspaceCard padded={false}>
        <ul className="divide-y divide-line">
          {incomingPending.map((row) => (
            <IncomingRequestRow key={row.id} row={row} />
          ))}
        </ul>
      </WorkspaceCard>
    </section>
...
function IncomingRequestRow({ row }: { row: PartnershipRow }) {
  return (
    <li className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 sm:px-6">
      ...
      <div className="flex gap-2">
        ...
        <form action={respondPartnership}>
          ...
          <input type="hidden" name="back" value="/dashboard/partners" />
          <Button type="submit" variant="ghost" className="h-9 px-3.5 text-[12px]">
            Decline
          </Button>
        </form>
      </div>
