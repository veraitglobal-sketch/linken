...
/** Google / Microsoft buttons — render nothing until a provider is enabled. */
export function OAuthButtons({ next }: { next: string }) {
  const { google, microsoft } = enabledOAuthProviders();
  if (!google && !microsoft) return null;

  const btn =
    "flex h-12 w-full items-center justify-center gap-3 rounded-full border border-line bg-surface text-[15px] font-semibold text-ink transition-colors hover:bg-mute";
