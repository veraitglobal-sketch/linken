...
-- ---------------------------------------------------------------------------
-- Trust level helper (mirrors src/features/trust/score.ts)
-- Returns: member | established | trusted | pillar
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Partner-confirmed only (no client confirmation) — approximate partner tags
...
-- No client INSERT/DELETE — matching functions only (service_role / definer)
...
-- ---------------------------------------------------------------------------
-- Does matched company satisfy a saved search?
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- ON-EVENT match: write feed rows for every search owner that matches
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- ON-READ backfill when a search is created (max 20 newest matches)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Digest helper (weekly cron TODO — max 1/week enforced in app layer)
-- ---------------------------------------------------------------------------
...
-- TODO(cron): weekly job calling get_radar_digest + email for radar firms.
-- Enforce max 1 email / 7 days per company in the job (no table yet for send log).
