-- A parent creating an unclaimed subsidiary already knows what it does —
-- let them say so instead of leaving "Services on record" empty and the
-- profile reading as a dead shell until someone claims it.

drop function if exists public.create_group_subsidiary(uuid, text, text, text, text, text, uuid, uuid, text);
...
revoke all on function public.create_group_subsidiary(uuid, text, text, text, text, text, uuid, uuid, text, text[]) from public;
grant execute on function public.create_group_subsidiary(uuid, text, text, text, text, text, uuid, uuid, text, text[]) to authenticated;
