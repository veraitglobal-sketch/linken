-- 1) plan is system-managed (no owner self-upgrade) — mirror verified protection.
-- 2) is_company_operator includes same-company admin (claimed or not).
...
-- Identity fields only — plan removed (handled separately below).
...
-- plan: same rules as verified — service_role / elevated session / explicit flag only.
