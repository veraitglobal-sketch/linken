-- Agent API owner-parity: expanded scopes + service-role bridge RPCs
-- (auth.uid() is null for service_role — Agent API authenticates the key instead).
...
-- ─── Rate limits for Agent API (no auth.uid) ─────────────────────────────
